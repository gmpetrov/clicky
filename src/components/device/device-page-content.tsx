...
export function DevicePageContent({ initialUserCode }: DevicePageContentProps) {
  const router = useRouter();
  const [userCode, setUserCode] = useState(formatUserCodeForDisplay(initialUserCode));
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          <label className="auth-label">
            ...
            <input
              ...
              value={userCode}
              onChange={(event) => {
                setUserCode(formatUserCodeForDisplay(event.target.value));
                setErrorMessage(null);
              }}
              type="text"
